...
public class TestAbLink extends BaseClass {
	...
	@Test
	public void verifyAddRemoveElementLink() {
		...
		WebElement element = wait.until(ExpectedConditions.elementToBeClickable(By.className("added-manually")));
		element.click();
		...
		if (btn == 1) {
			System.out.println("Test passed:Only one button exists");
		} else {
			System.out.println("Test failed:Delete button exists");
		}
	...
	@Test
	public void verifyHovers() throws InterruptedException {
		...
		Thread.sleep(1000);
		Assert.assertEquals(url, "http://the-internet.herokuapp.com/users/1");
